...
void LCDprintstr(char* str, int16_t y,uint16_t bkcolor, uint16_t charcolor)//240/8=30 символов в строке
{
    int len = strlen(str);
    if(len > 30)
        len=30;
	...
	uint16_t bstr[16*8*sizestr];//3840 точек * 2 байта = 7 680 байт = 7,5 Кб 
	int i,nchr,irow,icol;
    for(i=0;i<16*8*sizestr;i++)
		bstr[i]=bkcolor;
	for(nchr=0 ; nchr<len; nchr++)
	{
